...
namespace Lib
{
    public class Interface
    {
        ...
        public virtual void write_controlled_speed(int pos, Movement cmd, int speed)
        {
            if (!port.IsOpen) return;

            // Speed is entered in as deg/s
            // No need to handle error cases as the arduino code can catch out of bound things and use defaults
            string msg = pos.ToString() + ',' + cmd.GetHashCode().ToString() + ",s" + speed.ToString() + '\n';
